...
class Window extends JFrame
{
    ...
    /**
    Method: Window()
    Description: Constructor used to initilise the JFrame window with specific settings.
    Calls  drawInputNorthPanel(), drawCenterPanel(), drawButtonsSouthPanel() to draw different elements of the GUI
    */
    public Window()
    {
        super("ID List");
    ...
    /**
    Method: drawCenterPanelWithUserMessage()
    Description: Method is being called once a user enters an ID an presses "Insert ID to list" Button;
                 Adds a JLabel with relevant error message to the JPanel.
    Return: JPanel containing a JLabel with the relevant message in response to user input.
    */
    ...
    /**
    Method: drawCenterPanel()
    Description: Methods draws the center part of the Border Layout when 'Display all IDs' button is clicked
    Return: JPanel containing list of all IDS inside the array list.
    */
   ...
   /**
   Method: DrawInputNorthPanel()
   Description: Method draws the North part of the Border Layout when constructor is called
   Return: JPanel containing all input fields and labels
   */
    ...
    /**
   Method: drawButtonsSouthPanel()
   Description: Method draws the South part of the Border Layout when constructor is called;
                Creates an array of Buttons by calling setButtons() method;
                For loops through each button inside the list, styles it and adds it to JPanel
    ...
    */
    ...
    /**
    Method: setButtons()
    Description: Initialises all buttons required in the application;
                Adds Action Listener to each button and relevant implementation;
    ...
    */
    ...
    /**
   Method: styleIDLabel()
   Parameters: JPanel IDListPanelCenter; JPanel to add the label to
               JLabel tempLabelToStyle; Label to be styled
               Color C; Desired foreground color of the JLabel
   Description: Applies styles to a JLabel, then adds JLabel to JPanel passed as argument;
   Return: void
    */
    ...
    /**
    Method: fetchAndValidateIDInput()
    Description: Stores user input from text field inside a string variable;
                 Checks to see if input is empty;
                 Checks to see if input is a valid integer;
                 Checks to see if input is within 100000-999999 range;
                 Modifies ErrorInputMessage and sucessfulInputMessage variables accordingly.
    Return: int, returns user input if it is valid, else returns -1;
    */
    private int fetchAndValidateIDInput()
    {
        ...
        {
            ErrorInputMessage = "The ID '" + tempTextField + "' was not added to the list as it is not a valid ID. Please enter a 6 digit number!";
            return -1;
        }
    }
    ...
    /**
    Method: fetchColor()
    Description: Stores RBG values from input field inside int r, g, b variables;
               Checks to see if input is a valid integer;
               Checks to see if input is within 0-255 range;
    Return: returns color according to the r,g,b values passed by user. If input is invalid returns Color.BLACK
    */
    ...
    /**
    Method: repaintCentrePanelWithUserMessage()
    Description: updates Center Panel with relevant user message;
                 Removes the panel, re-creates it and then adds it to JFrame again;
                 Calls revalidate() and repaint() to update panel in real time.
    Return: void
    */
    ...
    /**
    Method: repaintCentrePanelWithUserMessage()
    Description: updates Center Panel with list of IDs once 'Display all IDS' button has been pressed;
                Removes the panel, re-creates it and then adds it to JFrame again;
                Calls revalidate() and repaint() to update panel in real time.
    Return: void
    */
...
public class CE203_RP19791_Ass1
{
    /**
     * main() : Calls Window constructor
     * @param args
     */
